...
- **P_them, P_us:** a masked team-lineup network (2×512 GELU, ensemble of 4) that sees only what a lobby shows.
  ...
  - Our own bans are interventions: they remove heroes but say nothing new about our players, so our side is averaged
    over the bans a typical team in our seat would have made (32 sampled histories).
...
- `tools/check_v7.js`, `tools/check_sim_v7.js`, `tools/check_sim_seeds.js`: the v7 verification (coherence, rollouts,
  information boundaries, stability, pairs, latency, changes against v6). Their latest output is in `tools/reports/`.
