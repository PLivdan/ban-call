...
opt = dict(NS=128, NOWN=16)
js = json.loads(subprocess.run([NODE, "tools/check_engine.js"], input=json.dumps(dict(lobbies=lobbies, opt=opt)), capture_output=True, text=True, check=True).stdout)
ref = Model(M, NS=opt["NS"], NOWN=opt["NOWN"]); bad = 0
